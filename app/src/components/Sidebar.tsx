...
/* Socials of the legacy (European) site. The «Китай» storefront links out only
   to university pages and the partner's Telegram, so the block is shown for the
  ...
  if (partner.logo) {
    return (
      <img
        src={partner.logo}
        alt={partner.name}
        className={cn("w-auto object-contain", size === "lg" ? "h-7" : "h-6")}
      />
    )
  }
  return (
        ...
        <button
          ...
          onClick={() => {
            if (withSub) {
              ...
              if (!isActive) setTab(PROGRAMS_SUBTABS[0].id)
            } else {
              setTab(it.id)
            }
          }}
